...
- **Références légales (Hyperliens imbriqués) :** Toutes les anomalies structurelles de type liens Markdown imbriqués (ex: `[[[[[l'article...]]]]]`) ciblant Légifrance ou Judilibre ont été corrigées avec précision via sed pour retrouver un format standard valide (`[Texte](URL)`) dans l'ensemble de l'arborescence `⚖️_Actes/` (fichiers courriers et archives) **sans altérer ni corrompre les URL ou la syntaxe Markdown**.
