---
title: "👤 Le Prénom du Préposé"
description: "Token :** `**[Le Prénom du Préposé]**`"
type: memory
---

<!-- Breadcrumb -->
*[🏠](../../README.md) › [🧠 Mémoire du Projet](../README.md) › [🗂️ Tokens — Fiches individuelles des tokens d'anonymisation](./README.md) › token-prepose-prenom*
<hr>
<!-- /Breadcrumb -->

# 👤 Le Prénom du Préposé

**Token :** `**[Le Prénom du Préposé]**`

## Identité

| Champ | Valeur |
|---|---|
| **Prénom réel** | Ayoub |
| **Personne associée** | [**[Le Préposé de l'Exploitation]**](token-exploitation-prepose-nom.md) |

## Contexte

Prénom seul du coiffeur préposé, mentionné textuellement dans les actes (témoignage). Le nom complet et les coordonnées sont anonymisés via le token [**[Le Préposé de l'Exploitation]**](token-exploitation-prepose-nom.md).

## Croisements

- ⇄ [Le Préposé de l'Exploitation](token-exploitation-prepose-nom.md)

- ⇄ [Le Téléphone du Préposé](token-exploitation-prepose-telephone.md)
