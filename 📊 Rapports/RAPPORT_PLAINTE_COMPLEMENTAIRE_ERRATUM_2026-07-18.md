---
title: "Rapport Préparation Dépôt Plainte Complémentaire et Erratum"
date: 2026-07-18
description: "Analyse et recommandations pour le dépôt au commissariat de Foix de l'erratum et de la plainte visant HB BARBER"
type: rapport
---
<!-- Breadcrumb -->
*[🏠](../README.md) › [📊 Rapports et Analyses](./README.md) › RAPPORT PLAINTE COMPLEMENTAIRE ERRATUM 2026-07-18*
<hr>
<!-- /Breadcrumb -->

<!-- Breadcrumb -->
*[🏠](../README.md) › [📊 Rapports](./README.md) › RAPPORT_PLAINTE_COMPLEMENTAIRE_ERRATUM_2026-07-18*
<hr>

# RAPPORT PRÉPARATION DÉPÔT PLAINTE COMPLÉMENTAIRE ET ERRATUM
## Mission d'Assistance Juridique — 18 juillet 2026

Ce rapport a pour objet de préparer le dépôt de la plainte complémentaire et de l'erratum de désignation au Commissariat de **[La Ville de l'Accident]** la semaine prochaine. Il synthétise l'analyse des pièces existantes et formule les recommandations nécessaires pour sécuriser cette démarche.

<hr><hr>

## 1. Analyse de l'Erratum (📜 Erratum Correction Identite Societe.md)

L'erratum actuel est structurellement solide et explique de manière satisfaisante la substitution de **[L'Exploitant du Commerce (La SAS)]** par **[Le Nouvel Exploitant (HB BARBER)]** (HB BARBER). Toutefois, une correction critique doit y être apportée.

### MENTIONS "DÉPOSÉ / ENVOYÉ" HALLUCINÉES
L'erratum actuel mentionne dans sa section 3 ("Actes déjà déposés/envoyés") des signalements administratifs (Inspection du Travail, URSSAF, Mairie, etc.).

**ATTENTION : L'exploration du dossier démontre que ces actes n'ont jamais été envoyés.** Ce sont des projets non transmis. Mentionner formellement ces envois comme réalisés dans une pièce déposée à la police constitue un risque de fausse déclaration.

**Recommandation d'action sur l'Erratum :**
- Supprimer de la liste (section 3.C) tous les signalements administratifs qui n'ont pas de preuve de dépôt (AR LRAR) effective.

- Ne conserver que les actes *réellement* envoyés et justifiés : Plainte initiale (2 juin), LRAR des 29 juin et 8 juillet, Saisine du Tribunal et CPAM.

<hr><hr>

## 2. Analyse du Projet de Déclaration PV (📋 Projet Declaration PV Foix.md)

Le projet de déclaration est à jour, précis, et intègre parfaitement les découvertes du 16 juillet 2026.

Il mentionne correctement :
- La découverte de l'enseigne HB BARBER.

- La disparition de la vasque en céramique cassée.

- Le refus de coopération de M. Hamza El Hachemi BERGUIGA (président de HB BARBER).

- La demande expresse de rectification pour viser HB BARBER au lieu de **[L'Exploitant du Commerce (La SAS)]**.

**Recommandation :** Le texte est prêt à être dicté à l'OPJ sans modification majeure.

<hr><hr>

## 3. Mise à jour du Bordereau de Pièces (📋 Bordereau de Pieces Foix.md)

Le bordereau de pièces existant est bien construit mais nécessite l'ajout explicite des preuves liées à la visite du 16 juillet 2026.

Voici les **ajouts obligatoires** à intégrer au bordereau actuel dans la section "PIÈCES NOUVELLES" :

- [ ] **Pièce à ajouter :** Photographies de la façade et de l'enseigne HB BARBER prises le 16/07/2026.

- [ ] **Pièce à ajouter :** Photographies (si existantes) de l'intérieur montrant l'absence de la vasque et les traces de fixation murales.

- [ ] **Pièce n°9 (Mise à jour) :** La Note de constatations du 16/07/2026 fait office de preuve de l'échange avec M. BERGUIGA (l'échange ayant été purement verbal et non par courriel). Elle doit être clairement présentée comme telle à l'OPJ.

<hr><hr>

## 4. Mémo de Synthèse pour l'OPJ

Ce mémo est destiné à être remis à l'OPJ pour fluidifier le dépôt et clarifier immédiatement le but de la démarche.

### MÉMO EXPLICATIF POUR LE SERVICE DE TRAITEMENT DES PLAINTES

**Objet :** Dépôt d'une déclaration complémentaire et d'un erratum formel
**Référence :** PV initial n° **[N° PV Police]** du 02/06/2026

**Pourquoi une nouvelle procédure / un complément ?**
La plainte initiale a été déposée avec incertitude sur l'identité de l'exploitant, sur la base d'informations erronées issues d'internet (désignant la société **[L'Exploitant du Commerce (La SAS)]**). De nouveaux éléments matériels justifient aujourd'hui de rediriger la procédure vers la société ayant réellement exploité le local au jour de l'accident (29 mai 2026).

**Ce qui change :**
La procédure ne doit plus viser la SAS **[L'Exploitant du Commerce (La SAS)]** (qui avait cessé son activité depuis le 10 mars 2026 selon le bailleur), mais la SAS **[Le Nouvel Exploitant (HB BARBER)]** (HB BARBER, SIREN **[SIREN du Nouvel Exploitant]**), créée le 22 avril 2026, seule responsable de la garde de l'équipement au jour du sinistre.

**Éléments nouveaux rapportés :**
- Visite des lieux du 16 juillet 2026 confirmant l'exploitation sous enseigne "HB BARBER".

- Constat de la disparition du matériel impliqué dans l'accident (vasque en céramique défectueuse).

- Refus de coopération, refus de communication d'assurance et absence de justification d'identité par le gérant actuel, M. Hamza El Hachemi BERGUIGA.

- Risque imminent de disparition des preuves matérielles et vidéo.

<hr><hr>

## 5. Démarches à accomplir au Commissariat ("Post-Dépôt")

Après avoir lu et fait enregistrer le PV de déclaration et l'erratum, il est impératif de s'assurer des points suivants avant de quitter les lieux :

- [ ] **Récépissé :** Demander et conserver précieusement le nouveau récépissé de dépôt de plainte / déclaration complémentaire.

- [ ] **Numéro d'enregistrement :** Noter le numéro de ce nouveau PV (s'il diffère du premier).

- [ ] **Vérification physique :** S'assurer visuellement que l'OPJ agrafe ou numérise formellement l'Erratum et la Note de constatations avec la nouvelle audition.

- [ ] **Demande de diligence :** Interroger l'OPJ sur la possibilité d'une enquête de proximité rapide pour figer les preuves (notamment vidéosurveillance et assurance), au vu des destructions de matériel constatées le 16 juillet.
